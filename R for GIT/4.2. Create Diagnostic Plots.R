...
ggplot(data = percentage_test_train,aes(x = cutoffs,y = percentage,color = traintest)) +
  ...
  labs(x=expression(atop(bold("Probability of Fishing Threshold"),atop(italic("Cutoff point for a prediction to be counted as fishing")))),y=expression(atop(bold("Percentage Correct"),atop(italic("of correctly identified pixels fishing")))),color = "Data Type") +
  ylim(c(0,100)) +
  scale_x_continuous(limits = c(0,1),breaks = seq(0,1,0.2)) +
  theme(axis.text=element_text(size=10),
        axis.title=element_text(size=14,face="bold")) +
  NULL
ggsave("./fishing/Future Prediction/paper//5 figures/final/in paper/Supplementary/Train_V_Test_Perc.png",width = 33.867,height = 19.05,units = "cm")
...
impact <- results_lst_NEMO[[1]][17:26,1]
impact_df <- data.frame(impact)
impact_df <- rownames_to_column(impact_df,var = "Variable")
ggplot(data = impact_df,aes(x = Variable,y = impact)) +
  geom_bar(position = "dodge",stat = "identity") +
